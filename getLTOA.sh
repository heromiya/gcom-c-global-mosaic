...
mkdir -p VRT/$RES
mkdir -p GCOM-C/composite/$RES/$VER
...
getRSRF() {
    ...
    H5FILE=GCOM-C/LTOA/GC1SG1_${YYYY}${MM}${DD}D01D_T${TILE}_L2SG_LTOAK_2002.h5
	...
	if [ $? -eq 0 ]; then
	    for B in VN04 VN06 VN07; do # Lt_VN10 VN03 VN05 VN07 
      		python3 h5_2_tiff.py $H5FILE Lt_${B} $H5FILE.$B.tif $GCP_INTERVAL
